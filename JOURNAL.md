...
## 2026-09-08 — session 10 (006 closed; content chores; 007-seo-foundations closed; PRODUCTION DEPLOY)

### Context

Continuing from session 9's pause. Covers, in one sitting: the deferred `massive-space` triage's final decision and `006`'s full lifecycle, a batch of out-of-band content chores the user requested while reviewing the site, a new `007-seo-foundations` feature (user-initiated: "¿Cómo estamos en SEO?") run end to end at "que quede mundial" (comprehensive) scope, and — for the first time this project — a real production deployment.

### Done this session

- **Massive-space final triage**: dispatched an Explore agent to investigate the 8 candidates deferred since session 7 (Calzatodo, Ficohsa/La Colonia, Pizzamania, Pepe Ganga, Juriscoop, Lilipink, Speedo, MassiveSpace Pro), verifying real authorship per repo via `git log` rather than trusting folder/client names — several "client" repos turned out to be majority-authored by other agency developers (Leonardo Parra/DevMassiveSpace on Calzatodo's/Ficohsa's/Lilipink's theme repos). Findings and the user's final selection recorded in `.specs/onboarding.md` (2026-09-08 entry): **discarded** for zero/insufficient attributable authorship — Pepe Ganga, Juriscoop, Speedo; **selected**, 4 of 5 remaining strong candidates — Calzatodo (`backend-services` only), Pizzamania (`auto-invoicer`, built with its own SDD+TDD trail), CrediPink (scoped to the credit product, not the LiliPink storefront), MassiveSpace Pro (presented as the user's own internal tool, no client named); Ficohsa/La Colonia **not selected**.
- **`006-massive-space-success-stories`, full lifecycle**: specify (9 criteria, 0 clarifications) → plan (no new dependency, extends `success-stories.ts`/`.en.ts`) → tasks (5 tasks) → implement (all closed, real R-G-F, content verified directly against each source repo — README/CLAUDE.md/composer.json/manifest.json — before drafting, per Article V) → verify (full report, no FAILs, confirmed in a real `nuxi generate` build). Closing commit `spec: 006 closed — verify green` (`8a9e7e5`). Site now has **23 success stories**.
- **Out-of-band content chores** (user-requested while reviewing the site, not tied to any spec): stale "Hechos" numbers (5+ years old) refreshed to approximate current estimates (38→65 clients, 43→95 projects, 750→1500 support hours, 15→30 complex projects), confirmed with the user before writing; added a new library write-up (`Filament General Settings`, evolution of the existing `GeneralSettings` package, real R-G-F + i18n keys); `MassiveSpace Pro`'s "checkout rápido/fast checkout" → "checkout custom/custom checkout" (single occurrence only, not touching Qbano's/Almacenes Brissa's own "checkout rápido" wording); Vultr author URL accent fixed (`josé-rafael-gutierrez` → `jose-rafael-gutierrez`); landline phone number removed from `data/contact.ts` (kept only the mobile number), with real R-G-F since two existing tests covered the old shape.
- **SEO audit** (user asked "¿Cómo estamos en SEO?"): found the site had **zero** `<title>`, meta description, Open Graph, Twitter Card, `robots.txt`, `sitemap.xml`, or structured data anywhere — confirmed directly in the real HTML served by the dev server, not guessed. hreflang/canonical (from `004`) were the only things already correct. Presented findings; user asked for a full `007` feature, explicitly "que quede mundial" (comprehensive, not a minimal patch).
- **`007-seo-foundations`, full lifecycle**, the most involved feature this session:
  - **Specify**: 11 criteria (title/description/OG/Twitter on all 8 pages, 404 `noindex` excluded from the sitemap, default OG image = existing profile photo — no new asset, `robots.txt`, sitemap, `Person` JSON-LD on home/about, representative-sample test coverage, a11y regression). Two genuine ambiguities resolved via `AskUserQuestion` before drafting: OG image source, 404 SEO treatment.
  - **Plan**: researched via context7 (`useSeoMeta` — Nuxt 3 core, no new dependency; `@nuxtjs/sitemap` — new dependency, justified). New **ADR 0006**: `@nuxtjs/sitemap` (with `zeroRuntime: true`) for the sitemap (auto-syncs with `@nuxtjs/i18n`'s route table, avoiding a third hand-maintained page list after it already drifted 3 times across `001`/`003`/`005`), but `robots.txt` stays hand-authored (3 lines that never change) — same "static file vs. dependency" question, different answer, because the two artifacts' content-change profile differs.
  - **Tasks**: 11 tasks (sitemap config → robots.txt → ES copy draft → EN copy draft → i18n keys → `usePageSeo()` composable → wire into 4+4 pages → `error.vue` noindex → `usePersonSchema()` → final a11y+build verification).
  - **Implement, three real "reality differed from plan" findings, all surfaced and documented as Amendments, not glossed over**:
    1. **T001**: `@nuxtjs/sitemap`'s automatic i18n integration produces `sitemap_index.xml` + one sitemap per locale, not a flat `sitemap.xml` (which turned out to be a static HTML meta-refresh redirect page — not valid XML, useless to a real crawler). Confirmed against the module's own docs before amending `spec.md`'s criteria 7-8 and both `contracts/` files to point `robots.txt` at `sitemap_index.xml` instead.
    2. **T006-T009**: `@unhead`'s DOM plugin flushes `<title>`/meta tag updates asynchronously (debounced) in the test environment — every new test needed `vi.waitFor` instead of asserting immediately after `mountSuspended`. `error.vue`'s locale in tests also turned out to leak from whichever route a prior test in the same file last mounted (it renders outside normal navigation), so that test asserts what criterion 5 actually requires (a real title, `robots: noindex`) rather than one exact locale's copy.
    3. **T010**: extracted the `Person` JSON-LD object-building logic into `utils/person-schema.ts` (pure function, real classicist unit test, no Nuxt bootstrap) instead of one Nuxt-coupled composable as `plan.md` sketched — matches the project's established `utils/dates.ts`/`utils/heatmap.ts` pattern. The route-gate test (JSON-LD only on home/about) needed two separate spec files, not one: `@unhead` dedupes `<script>` tags by content, so a single-file "mount home, remove the tag, mount contact, assert absence" test passed even with the gate forced open — caught by deliberately forcing the gate open first and confirming the absence test then failed, before trusting it.
  - **Verify**: full report, no FAILs (see `007-seo-foundations/spec.md`'s Closed footer) — all 11 criteria cite their `spec-ref` by number, no citation gaps this time (unlike 001/003/004/006). One R9 fix: README.md didn't mention the new sitemap/robots.txt build output, added a line. Confirmed directly in a real `nuxi generate` build (41 routes): `robots.txt`, `sitemap_index.xml` + 2 per-locale sitemaps (16 URLs total), title/description/OG/Twitter/JSON-LD spot-checked on home/about in both locales. Closing commit `spec: 007 closed — verify green` (`a5b1762`).
- **First production deployment of the Nuxt 3 rewrite**, user-initiated ("vamos a desplegarlo"), to the existing infrastructure (VPS `vps_josefo_01`, `~/www/hv`, `hv-container`, orchestrated by `~/www/nginx-proxy/docker-compose.yml` — a shared nginx-proxy + letsencrypt setup fronting several of the user's other sites). Presented a 6-step plan (push → pull → build → recreate → verify → rollback plan) before touching anything; user approved. Found the live site was still the pre-`001` Vue 2 build (`origin/master` had never received the rewrite — 275 commits sat local-only since session 1) and `hv-container` referenced a pre-built image by tag, not a `build:` directive, so deploying meant building the image manually rather than a compose-triggered build:
  1. `git push origin master` (275 commits, `dbaef81..29c1b74`).
  2. `git pull` on the VPS — clean fast-forward, the entire Vue 2 tree replaced by the Nuxt 3 one in one step.
  3. `docker build -t josefo727/hv:latest .` on the VPS (multi-stage: Node 20 builder, nginx-alpine runtime) — new image 75MB vs. the old 378MB (no Node/npm shipped, per `001`'s T025 design). The old image (`e2a072c066ec`, 23 months old) was left in place, untagged but not deleted, as an immediate rollback path.
  4. `docker compose up -d --force-recreate hv` from `~/www/nginx-proxy` — recreated `hv-container` on the new image; `nginx-proxy`/`letsencrypt` untouched, existing TLS cert for `hv.jose-gutierrez.com` reused as-is.
  5. Verified live: all 8 routes × 2 locales return 200, `/does-not-exist` returns a real 404 (not silently falling back to the homepage), `robots.txt`/`sitemap_index.xml` serve correctly, title/OG tags and the new success stories confirmed in the actual HTTPS response, HSTS header present.
- **`hv.jose-gutierrez.com` is now serving the full `001`-`007` rewrite in production** — the first time any of this session's (or the prior nine sessions') work has been live rather than local-only.

### Open

- Nothing queued for `.specs/onboarding.md` — the `massive-space` deferred-candidate list is now fully resolved (4 added in `006`, 3 discarded, 1 not selected).
- The old `hv` Docker image (`e2a072c066ec`, 378MB) is still on the VPS, untagged, as a rollback path — safe to prune later once the new deploy is confirmed stable, but left alone for now; nobody asked for cleanup.
- `prompt.md` (the session 1 original task prompt, untracked since the very first commit) is still sitting untracked in the repo root — never addressed, low priority.
...
- `.specs/onboarding.md` → Decisions log (final massive-space triage, 2026-09-08).
- `.specs/006-massive-space-success-stories/spec.md`, `plan.md`, `tasks.md` → Closed footer, full verify report, verbatim approved drafts.
- `.specs/007-seo-foundations/spec.md` → Amendments (sitemap_index.xml reality, 404 CSR-shell context), Closed footer, full verify report.
- `.specs/adr/0006-sitemap-module.md`.

### Dead ends / discarded

- A single-file "mount home, remove the JSON-LD tag, mount contact, assert absence" test for `usePersonSchema()`'s route gate: passed even with the gate forced open, because `@unhead` dedupes `<script>` tags by content — discarded in favor of two separate spec files, each a fresh mount with no manual DOM manipulation. Caught before trusting it, not after.
- Assuming a flat `sitemap.xml` (per the original `007` plan/contract) as the real crawlable artifact: `@nuxtjs/sitemap`'s i18n auto-integration makes it a static redirect page instead once 2+ locales are detected — corrected via Amendment before writing more code around the wrong assumption.

### Resume from

Nothing specific queued. The portfolio is live at `https://hv.jose-gutierrez.com/` with all of `001`-`007`. Next session starts open — likely candidates per earlier notes: prune the old Docker image on the VPS if desired, decide whether to address `prompt.md`, or pick a new feature/content pass. Dev server (`npm run dev`) was left running locally this session; check with `curl localhost:3000` or restart if needed.
